...
const dotenv = require("dotenv").config();      // This allows us to use the .env file for required spotify information/
const axios = require("axios");                 // This allows us to use axios to make calls to an API
const moment = require("moment");               // This allows us to change the date formatting (moment has other features as well)
const Spotify = require("node-spotify-api");    // This allows us to use the node spotify api
const fs = require('fs');                       // This allows us to read from/write to other files

// Add the code required to import the keys.js file and store it in a variable.
const keys = require("./keys.js");

// Store key value for API keys 
const spotify = new Spotify(keys.spotify) // This pulls from the "keys.js" file where there is an object called Spotify. 
...
const liriapp = function(theCommand, data) { // This function takes in 2 parameters: a command and data. The command comes from the user's command, and data comes from what the user enters after their command //

switch (theCommand) {  // The switch happens depending on the command given
